...
# Проверка на DCFl 
Проверим слово на DCFL 
$w_1= S \to TTT  \to  SSTTT  \to  a^nSa^nSTT \to a^naba^nabbaba$
$w_2= S\to aSa \to a^{n}Sa^{n} \to a^{n}SSTTa^{n} \to a^nabSbabaa^n \to a^naba^naba^nbabaa^n$ 
$x = a^naba^{n}a$
пересечем слово с регуляркой  $a^*aba^*ab (baba|a^\*babaa^\*)$ 
1) Рассмотрим накачку в X . т.к в w2 каждый блок $a^n$ не связаны друг с другом то любое изменение в этих блоках выводит нас с языка .  и т.к разбиение этого слово единственное т.е нет случая когда первые два a^n связаны т.е выходит из одной репродукции по правилу S->aSa. То любая накачка в x выводит из w2 .
2) т.к p<n  x2 всегда лежит в втором блоке a^n тогда w1 выводится с языка при любой накачке 

оба условия выполнены   $\to L \notin DCFl$
